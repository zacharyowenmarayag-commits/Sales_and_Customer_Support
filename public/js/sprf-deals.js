(function () {
    const dealsPage = document.getElementById('sprf-deals-page');
    const availableMonthsAttr = dealsPage?.getAttribute('data-available-months');
    window._sprfAvailMonths = availableMonthsAttr ? JSON.parse(availableMonthsAttr) : [];
...
(function () {
    ...
    function fmtDate(d, includeYear = true) {
    ...
    function fmtRange(s, e) {
        ...
        if (!e) return fmtDate(s, false) + ' — pick end date';
        return fmtDate(s, false) + ' - ' + fmtDate(e, true);
    }
    ...
    function renderMonthGrid(popupId, grid) {
        ...
        grid.querySelectorAll('.cal-nav-btn').forEach(btn => {
            btn.addEventListener('click', () => window.calNav(popupId, parseInt(btn.dataset.nav)));
        });
        grid.addEventListener('click', function (e) {
            const target = e.target.closest('[data-pick="day"]');
            if (!target) return;
            const wrap = target.closest('.cal-day-wrap');
            if (wrap) window.calPickDay(popupId, parseInt(wrap.dataset.y), parseInt(wrap.dataset.m), parseInt(wrap.dataset.d));
        });
        grid.addEventListener('mouseover', function (e) {
            const wrap = e.target.closest('.cal-day-wrap');
            if (!wrap) return;
            window.calHover(popupId, parseInt(wrap.dataset.y), parseInt(wrap.dataset.m), parseInt(wrap.dataset.d));
        });
        grid.addEventListener('mouseleave', function () { window.calLeave(popupId); });
    }
    ...
    function renderQuarterGrid(popupId, grid) {
        ...
        grid.querySelectorAll('.cal-nav-btn').forEach(btn => {
            btn.addEventListener('click', () => window.calNavYear(popupId, parseInt(btn.dataset.navYear)));
        });
        grid.querySelectorAll('.cal-quarter-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                window.calPickRange(popupId, parseInt(btn.dataset.qsy), parseInt(btn.dataset.qsm), 1, parseInt(btn.dataset.qey), parseInt(btn.dataset.qem), 0);
            });
        });
    }
    ...
    function renderYearGrid(popupId, grid) {
        ...
        grid.querySelectorAll('.cal-year-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                window.calPickRange(popupId, y, 0, 1, y, 11, 31);
            });
        });
    }
